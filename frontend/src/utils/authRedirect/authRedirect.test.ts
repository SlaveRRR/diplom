...
describe('authRedirect utils', () => {
  ...
  test('возвращает label для intent и fallback', () => {
  ...
  test('собирает текущий относительный url', () => {
    expect(getCurrentRelativeUrl('/catalog', '?genre=1', '#top')).toBe('/catalog?genre=1#top');
  });

  test('валидирует redirect и отбрасывает auth pages и внешние url', () => {
  ...
  test('строит auth path с redirect и intent', () => {
  ...
  test('читает redirect и intent из query string', () => {
  ...
  test('сохраняет и одноразово потребляет pending redirect', () => {
  ...
  test('очищает pending redirect для небезопасного пути', () => {
